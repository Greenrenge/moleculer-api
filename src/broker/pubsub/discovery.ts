...
export class DiscoveryPubSub extends PubSub<{
  ...
  public async subscribeAll(listeners: { connected: (service: Readonly<Service>) => void; disconnected: (service: Readonly<Service>) => void; nodePoolUpdated: (service: Readonly<Service>) => void }): Promise<void> {
    await Promise.all([this.subscribe("connected", listeners.connected), this.subscribe("disconnected", listeners.disconnected), this.subscribe("nodePoolUpdated", listeners.nodePoolUpdated)]);
  }
}
