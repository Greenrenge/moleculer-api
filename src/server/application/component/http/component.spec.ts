...
describe("http application should work with routes", () => {
  ...
  it("httpClient should got proper response", () => {
    return expect(fetch("http://localhost:8889/data", { method: "GET" }).then((res) => res.json())).resolves.toMatchObject(message);
  });

  it("httpClient should mount all given prefixes", () => {
    return expect(fetch("http://localhost:8889/~dev/data", { method: "GET" }).then((res) => res.json())).resolves.toMatchObject(message);
  });
